...
export function GlobalNavigation({ 
  ...
  const navItems = [
    { id: 'landing', label: 'Home', action: onNavigateHome },
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative">
          ...
          {isDesktop && (
           ...
           {!isDesktop && (
              <div className="relative" ref={navMenuRef}>
                {showNavMenu && (
                  <div className={`absolute left-0 top-0 min-w-[200px] border rounded-xl shadow-2xl p-2 space-y-1 animate-fadeIn z-50 ${isDarkMode ? 'bg-[#0C0F16] border-interactive' : 'bg-white border-sky-200'}`}>
                    {navItems.map(item => {
                      const isActive = currentView === item.id;
                      return (
                        <button
          ...
          <div className="flex items-center gap-3">
             {/* Desktop Breadcrumbs (optional, if room permits or hide them) */}
            ...
            <div className="flex items-center gap-2 text-xs">
              <span className={`hidden xl:inline uppercase tracking-widest ${isDarkMode ? 'text-neutral-500' : 'text-slate-400'}`}>Brand</span>
              <span className={`
