...
export const ButtonGeneratorWidget = ({ config, onConfigChange, onCustomizeRequest, brandTheme }) => {
  ...
  const showBuilderUI = config.builderUI !== false;
  ...
  const handleAddButton = () => {
    setButtons((prev) => [...prev, { ...initialButton, id: nanoid() }]);
  };

  const handleDuplicateButton = (id) => {
    setButtons((prev) => {
      const buttonToDuplicate = prev.find(b => b.id === id);
      if (!buttonToDuplicate) return prev;
      return [...prev, { ...buttonToDuplicate, id: nanoid() }];
    });
  };
  ...
  const handleDeleteButton = (id) => {
    ...
    setButtons((prev) => {
      const nextButtons = prev.filter(button => button.id !== id);
      if (activeButtonId === id) {
        setActiveButtonId(nextButtons[0]?.id || null);
      }
      return nextButtons;
    });
  };
  ...
  const handleMoveButton = (id, direction) => {
    setButtons((prev) => {
      ...
      const nextButtons = [...prev];
      if (direction === 'up' && index > 0) {
        [nextButtons[index - 1], nextButtons[index]] = [nextButtons[index], nextButtons[index - 1]];
      } else if (direction === 'down' && index < nextButtons.length - 1) {
        [nextButtons[index + 1], nextButtons[index]] = [nextButtons[index], nextButtons[index + 1]];
      }
      return nextButtons;
    });
  ...
  const handleCopyStyleToOthers = (id) => {
    setButtons((prev) => {
      const sourceButton = prev.find(b => b.id === id);
      if (!sourceButton) return prev;
      const { id: _ignoredId, label: _label, url: _url, ...styleToCopy } = sourceButton;
      return prev.map(button => button.id === id ? button : { ...button, ...styleToCopy });
    });
  };

  const openManager = (buttonId = null) => {
    setShowManager(true);
    const nextId = buttonId || activeButtonId || buttons[0]?.id || null;
    if (nextId) setActiveButtonId(nextId);
  };
    ...
    >
      {showBuilderUI && (
        <div className="absolute top-4 right-4 z-20 flex items-center gap-2">
          <button
            type="button"
            className="text-[11px] px-3 py-1.5 rounded-full border border-white/20 bg-black/20 text-neutral-200 hover:border-purple-300 hover:text-white transition"
            onClick={() => openManager()}
          >
            Manage
          </button>
          <button
            type="button"
            className="text-[11px] px-3 py-1.5 rounded-full border border-purple-400/60 bg-purple-500/15 text-white hover:bg-purple-500/25 transition flex items-center gap-1.5"
